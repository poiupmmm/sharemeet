...
export async function getActivity(id: string) {
  ...
  try {
    ...
    const activity: Activity = {
      ...
      start_time: data.start_time || data.date, // 使用start_time字段
      end_time: data.end_time || data.end_date || data.start_time || data.date, // 确保有结束时间
...
export async function createActivity(activity: Omit<Activity, 'id' | 'created_at' | 'participants_count'>) {
  ...
  try {
    ...
    const formattedActivity = {
      ...
      start_time: activity.start_time, // 使用start_time字段
      end_time: activity.end_time, // 使用end_time字段
...
export async function joinActivity(activityId: string): Promise<{ success: boolean; message?: string; error?: any; needLogin?: boolean }> {
  ...
  try {
    ...
    console.log('从Cookie中获取登录状态:', { isLoggedIn, userId, cookies });
    
    if (!isLoggedIn || !userId) {
      console.log('Cookie中未找到登录状态或用户ID，尝试从Supabase会话获取');
      
      // 尝试从Supabase会话获取用户信息
      try {
        const { data: { session } } = await supabase.auth.getSession();
        if (session && session.user) {
          console.log('从Supabase会话获取到用户信息:', session.user.id);
          // 执行实际的加入活动逻辑
          return await processJoinActivity(supabase, activityId, session.user.id);
        } else {
          console.log('Supabase会话中也没有用户信息，需要登录');
          return { 
            success: false, 
            needLogin: true, 
            message: '您需要登录才能参加活动' 
        ...
        console.error('获取Supabase会话出错:', sessionError);
        return { 
          success: false, 
          needLogin: true, 
          message: '您需要登录才能参加活动' 
  ...
  try {
    ...
    if (participations && participations.length > 0) {
      ...
      const { data: activities, error: activitiesError } = await supabase
        ...
        .select('*')
        .in('id', activityIds)
        .neq('user_id', userId); // 排除用户自己创建的活动，避免重复
    ...
    // 4. 合并两种活动并添加标记
    const createdActivitiesWithMeta = (createdActivities || []).map(activity => ({
      ...activity,
      is_joined: false, // 用户是创建者，不是参与者
      is_creator: true
    }));
    
    const participatedActivitiesWithMeta = participatedActivities.map(activity => ({
      ...activity,
      is_joined: true, // 用户是参与者
      is_creator: false
    }));
    
    // 合并两类活动
    const allActivities = [
      ...createdActivitiesWithMeta,
      ...participatedActivitiesWithMeta
    ];
